...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `shellman`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in shellman.__all__ or not hasattr(shellman, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in shellman.__all__:
...
def test_inventory_matches_api(
    ...
    for item in inventory.values():
        if (
            ...
            and (item.name == "shellman" or item.name.startswith("shellman."))
        ):
